...
reg  [1:0]    scan_indicator;
reg [31:0]    cyc1, cyc2, cyc3, cyc4;
reg  [3-1: 0] ren_dly ;
reg           ack_dly ;
reg [1:0] SM = 2'd0;
reg [2:0] SM2 = 2'd0;
...
if (dac_rstn_i == 1'b0) begin
   ...
   if (sys_wen) begin
      ...
      if (sys_addr[19:0]==20'h44)   scan_indicator  <= sys_wdata[1:0] ; // start trigger for scan amplitude
    ...
    if(scan_indicator[0]) 
        start_of_scan <= 1'b1; //send from MS1
    if(scan_indicator[1])  
        start_of_scan <= 1'b0;
    ...
    case(SM2)
        3'd0 :
            begin
                ...
                counter4 <= cyc4;
                integrator_measure_reg <= {3{1'b0}};
                if(start_of_scan && amp_change_strobe)
                    SM2 <= 3'd1;
                else 
                    SM2 <= 3'd0;
           end
...
//                                    counter2 <= cyc2;
        ...
        3'd3 :
            begin
                ...
                integrator_ctrl[0] <= 1'b0;
                integrator_ctrl[1] <= 1'b1;
//                integrator_measure_reg <= {integrator_measure_reg[1:0], 1'b1};
